...
def entropy(class_y):
    ...
    if count_0 == total_count or count_1 == total_count:
        return 0

    p0 = float(count_0) / total_count
    p1 = float(count_1) / total_count

    entropy_res = -p0 * np.log2(p0) - p1 * np.log2(p1)
...
def information_gain(previous_y, current_y):
    ...
    for cur in current_y:
        entropy_cur += len(cur) / n * entropy(cur)
